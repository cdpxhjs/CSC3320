...
int main()
{
    ...
    {
        if(letter[j]>first_max && letter[j]>second_max)// if the number is larger than both first max number and second max number
        {
    ...
    for (k = 0; k < length1; k++)
    {
        if((content[k]>='a' && content[k]<='z') || (content[k]>='A' && content[k]<='B'))
        {
            if(content[k]-key>=97)//if current ASCII code is still larger than 'a'
            {
                content[k] -= key;
            }
            else
            {
                content[k] = content[k] + 26 - key;
            }
        }
    }
    printf("%s", content);
